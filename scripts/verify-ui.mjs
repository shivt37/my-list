...
check("cards use card-row layout", doc.querySelectorAll(".list-card .card-row").length === doc.querySelectorAll(".list-card").length);
check("card has url input + type select + max-pages + delete", (() => {
  const row = doc.querySelector(".list-card .card-row");
